...
FOR EACH ROW BEGIN
  ...
  VALUES (
    ...
    NULL,
    JSON_OBJECT('id_usuario', NEW.`id_usuario`, 'id_empleado', NEW.`id_empleado`, 'id_dependencia', NEW.`id_dependencia`, 'usuario', NEW.`usuario`, 'password', NEW.`password`, 'rol', NEW.`rol`, 'estado', NEW.`estado`),
    CURRENT_USER(),
    SUBSTRING_INDEX(USER(), '@', -1),
    NOW(),
    NOW()
...
FOR EACH ROW BEGIN
  ...
  VALUES (
    ...
    JSON_OBJECT('id_usuario', OLD.`id_usuario`, 'id_empleado', OLD.`id_empleado`, 'id_dependencia', OLD.`id_dependencia`, 'usuario', OLD.`usuario`, 'password', OLD.`password`, 'rol', OLD.`rol`, 'estado', OLD.`estado`),
    JSON_OBJECT('id_usuario', NEW.`id_usuario`, 'id_empleado', NEW.`id_empleado`, 'id_dependencia', NEW.`id_dependencia`, 'usuario', NEW.`usuario`, 'password', NEW.`password`, 'rol', NEW.`rol`, 'estado', NEW.`estado`),
    CURRENT_USER(),
    SUBSTRING_INDEX(USER(), '@', -1),
    NOW(),
